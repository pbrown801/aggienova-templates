...
def initialize_plots(plot, output_file_name):
    ...
    ax3 = " "
    files = " "
    if os.path.exists(os.path.join('..', 'uvot', 'animation_images')) and os.path.exists(
            os.path.join('..', 'images', 'website_images')):
    ...
    for i in range(num_groups):
        groups[i]['Wavelength'], groups[i]['Flux'] = zip(*sorted(zip(groups[i]['Wavelength'], groups[i]['Flux'])))
        if i == 0:
            ax1.plot(groups[i]['Wavelength'], groups[i]['Flux'], 'wo', markersize=1,
                     label=" " * (len(str(time_groups[i])) + 6))

        else:
            ax1.plot(groups[i]['Wavelength'], groups[i]['Flux'], 'wo', markersize=1)

    # Plot empty plots for each time that will be used by funcAnimation and set_data function to plot the data at
    # certain frames
    times_plots = []
    for i in range(num_groups):
        time_var = 'time' + str(i)
        time_var, = ax1.plot([], [], '-', markersize=0.3)
    ...
    # The combined data is simply each row is the appropriate time the data was measured and the associated
    # band magnitude measurements
    df = pd.read_csv('../output/MAGS/' + output_file_name + '_mangledmagsarray.csv')

    # Interpolate linearly for the missing NaN values in the each band that has Nan values. We do not do it for band
    # error measurements
...
# ------------------------ Animation Function --------------------
def animation(plot, fig, ax1, ax2, ax3, times_plots, groups, time_groups, bands_plots, df, filter_bands, num_groups,
              files, interval_time, amt_image):
    def init():
    ...
    def update(i):
        if i == num_groups:
            ...
            if ax3 != " ":
                if not (amt_image) and (len(files_png) != 0):
    ...
    return FuncAnimation(fig, update, init_func=init, frames=np.arange(0, num_groups + 1), interval=interval_time,
                         repeat=True)


def static_plots(plot, fig, ax1, ax2, ax3, times_plots, groups, time_groups, bands_plots, df, filter_bands, num_groups,
                 files, amt_image):
    if ax3 != " ":
        ax3.set_xticks([])
        ax3.set_yticks([])
...
def summary_plot(plot, output_file_name, save, show, amt_image=True, interval_time=100, type_plot=False):
    '''
    Summary Plot to show spectrum and light curve of input supernovae. 
    Input parameters:
        plot- (string) Supernovae name (i.e SN2007af)
        output_file_name - (string) : The output file name that we make in run_pipeline. A combination of the SN name+template spectra used (i.e SN2007af_SNII_series).
        save             - (bool)   : True=Save, False=Don't Save - Save plot to output/PLOTS/ directory.
        show             - (bool)   : True=Display, False=Don't Display - NOTE: For some supernovae this may take some time to load.
        amt_image        - (bool)   : True=single image, False=many images -  Animation images are in uvot folder. If single image is selected with animation images the last one is displayed statically.
        interval_time    - (int)    : Animation duration - Default 100.
        type_plot        - (bool)   : True=Static plot, (Default) False=Animation plot
    '''
    ...
    if (type_plot):
        ...
        anim = animation(plot, fig, ax1, ax2, ax3, times_plots, groups, time_groups, bands_plots, df, filter_bands,
                         num_groups, files, interval_time, amt_image)
        if save:
            print("Saving summary plot for", output_file_name)
            anim.save(r'../output/PLOTS/' + output_file_name + '_animation.gif', writer="Pillow")
            fig.savefig(r'../output/PLOTS/' + output_file_name + '_summaryPlot.png')
    if show:
...
if __name__ == "__main__":
    pass
    # summary_plot("SN2005cs","SN2005cs_uvot_SNIa_series", False, True, False, 500, False)
    # summary_plot("SN2007af","SN2007af_SN2017erp_m1_UVopt.dat", False, True, True, 500, False)
    # summary_plot("SN2007af","SN2007af_SN2017erp_m1_UVopt.dat", False, True, True, 500, True)
    # summary_plot("SN2007af","SN2007af_SNII_series", False, True, True, 500, False)
