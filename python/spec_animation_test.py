...
# ------------------------ Animation Function --------------------
def animation(plot, fig, ax1, ax2, ax3, times_plots, groups, time_groups, bands_plots,df, filter_bands, num_groups, files, interval_time, amt_image):
        ...
        if ax3 != " ":
            ...
            try:
                if amt_image:
                    ...
                    if (len(files_png)==0):
                        print("No image file for the supernova. Try to use True for the amt_img parameter to get a single image.")
                    else:
...
def static_plots(plot, fig, ax1, ax2, ax3, times_plots, groups, time_groups, bands_plots,df, filter_bands, num_groups, files, amt_image):
    if ax3 != " ":
            ax3.set_xticks([])
            ax3.set_yticks([])
...
def summary_plot(plot, output_file_name, save, show, amt_image=True, interval_time=100, type_plot=False):
    ...
    if(type_plot):
        static_plots(plot, fig,ax1, ax2, ax3, times_plots, groups, time_groups, bands_plots,df, filter_bands, num_groups, files, amt_image)
        if save:
            print("Saving summary plot for", output_file_name)
            fig.savefig(r'../output/PLOTS/'+output_file_name+'_summaryPlot.png')
    else:
        anim=animation(plot, fig,ax1, ax2, ax3, times_plots, groups, time_groups, bands_plots,df, filter_bands, num_groups, files, interval_time, amt_image)
        if save:
            print("Saving summary plot for", output_file_name)
