...
import argparse
from observedmags_to_counts_2 import *
...
'''
Use the template spectrum to create a mangled flux spectrum along the template spectrum wavelength. 
The mangled flux spectrum is used with the mangled spectrum wavelength to create an interpolation function that is applied
to the interpolated wavelength range
'''
def mangle_data(file, pivotlist, template_spectrum, filter_file_list, reader, reference_epoch_mjd, zeropointlist):
    ...
    for row in reader:
        ...
        # The count rrates from the counts_array file of the supernova which is the converted magnitudes from the original observations of each epoch of each band
