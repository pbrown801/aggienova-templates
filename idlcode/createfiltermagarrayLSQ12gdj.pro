pro createfiltermagarrayLSQ12gdj
...
filters=['$SNSCRIPTS/filters/UVW2_B11.txt','$SNSCRIPTS/filters/UVM2_B11.txt','$SNSCRIPTS/filters/UVW1_B11.txt','$SNSCRIPTS/filters/U_P08.txt','$SNSCRIPTS/filters/B_P08.txt','$SNSCRIPTS/filters/V_P08.txt','$SNSCRIPTS/filters/r_passband_period_one_and_two.txt','$SNSCRIPTS/filters/i_passband_period_one_and_two.txt','$SNSCRIPTS/filters/J_2mass.txt','$SNSCRIPTS/filters/H_2mass.txt']
...
readcol, 'lc.naturalsystem.sesn_allphot.dat', cfaName, cfaBand, cfaMJD, cfaNatMag,  cfadMag,   cfaCampaign,  format='(A, A, F, F, F, A)', /silent

vlines=where(cfaName eq shortname and cfaband eq 'V')
blines=where(cfaName eq shortname and cfaband eq 'B')
rlines=where(cfaName eq shortname and cfaband eq "r'")
ilines=where(cfaName eq shortname and cfaband eq "i'")
jlines=where(cfaName eq shortname and cfaband eq 'J')
hlines=where(cfaName eq shortname and cfaband eq 'H')
klines=where(cfaName eq shortname and cfaband eq 'K')
;; GRB time 2006-02-18 03:33:36   MJD 53784.14833333
; first epoch is set at B=V=24
; last epoch is from Maeda et al. 2007

allbmjds=[53784.14833333, transpose(dt.bb[0,where(finite(dt.bb[1,*]) eq 1)]), cfamjd[blines[where(finite(cfanatmag[blines]) eq 1)]]]
allbmags=[24.0, transpose(dt.bb[1,where(finite(dt.bb[1,*]) eq 1)]), cfanatmag[blines[where(finite(cfanatmag[blines]) eq 1)]]]
allbmagerrs=[0.2, transpose(dt.bb[2,where(finite(dt.bb[1,*]) eq 1)]), cfadmag[blines[where(finite(cfanatmag[blines]) eq 1)]]]
...
allvmjds=[53784.14833333, transpose(dt.vv[0,where(finite(dt.vv[1,*]) eq 1)]), cfamjd[vlines[where(finite(cfanatmag[vlines]) eq 1)]], 53995.0]
allvmags=[24.0, transpose(dt.vv[1,where(finite(dt.vv[1,*]) eq 1)]), cfanatmag[vlines[where(finite(cfanatmag[vlines]) eq 1)]], 22.45]
allvmagerrs=[0.2, transpose(dt.vv[2,where(finite(dt.vv[1,*]) eq 1)]), cfadmag[vlines[where(finite(cfanatmag[vlines]) eq 1)]], 0.4]
...
jgood=where(finite(cfanatmag[jlines]) eq 1, jcount)
jorder=sort(cfamjd[jlines[jgood]])
for i=0,jcount-1 do filtermjdarray[8,i]=   cfamjd[jlines[jgood[jorder[i]]]]
for i=0,jcount-1 do filtermagarray[8,i]=   cfanatmag[jlines[jgood[jorder[i]]]]
for i=0,jcount-1 do filtermagerrarray[8,i]=   cfadmag[jlines[jgood[jorder[i]]]]


hgood=where(finite(cfanatmag[hlines]) eq 1, hcount)
horder=sort(cfamjd[hlines[hgood]])
for i=0,hcount-1 do filtermjdarray[9,i]=   cfamjd[hlines[hgood[horder[i]]]]
for i=0,hcount-1 do filtermagarray[9,i]=   cfanatmag[hlines[hgood[horder[i]]]]
for i=0,hcount-1 do filtermagerrarray[9,i]=   cfadmag[hlines[hgood[horder[i]]]]


;kgood=where(finite(cfanatmag[klines]) eq 1, kcount)
;korder=sort(cfamjd[klines[kgood]])
;for i=0,kcount-1 do filtermjdarray[10,i]=   cfamjd[klines[kgood[korder[i]]]]
;for i=0,kcount-1 do filtermagarray[10,i]=   cfanatmag[ilines[kgood[korder[i]]]]
;for i=0,kcount-1 do filtermagerrarray[10,i]=   cfadmag[klines[kgood[korder[i]]]]
